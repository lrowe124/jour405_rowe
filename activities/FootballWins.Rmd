...
ANSWER HERE
I think that this is a useful model because the significance of various coefficients seems to line up with what I believe would increase the likelihood fo winning a game. Points scored, for example, has a p value of .00665 which makes it statistically significant. This makes sense to me when I think about what would help win a football game. Additionally, the adjusted R-squared value for the model is 58.28%, which means that 58.28%  of the variance in the model is explained by the model after accounting for the number of variables used.
...
ANSWER HERE
Points scored has the most impact on wins, as it correlates the strongest with wins as shown in both models. Yards gained seems to have the least impact on wins, as its correlation with both wins as well as other factors is comparatively weak.
## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

Recent analysis of multiple factors including yards gained, takeaways and points gained revealed that points scored had the strongest impact on whether or not a team won a football game. Conversely, yards gained to have least impact on whether or not a team will win.