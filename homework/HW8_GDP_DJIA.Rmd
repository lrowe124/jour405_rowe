...
```{r}
Measures <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
Measures |> summarize(correlation = cor(GDP, DJIA, method="pearson"))
```
...
Measures |> 
  ggplot() +
  geom_point(aes(x=DJIA, y=GDP)) + labs(title = "Correlation Between DJIA and GDP")
...
The two variables seem to be quite strongly correlated. The scatter plot shows a strong positive relationship because both variables increase within the graph. The correlation coefficient is also very strong at 0.9531058. Even though the left side of the graph is more uniform than the right, both sides show positive correlation.
