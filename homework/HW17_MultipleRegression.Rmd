...
ANSWER HERE
Income seems to be strongly correlated with a having a bachelors degree or higher, with a value of .75. It also seems to be correlated somewhat to turnout, at around .45. This is not very significant, but compared to other coreelations it is pretty high. the percentage of being white also seems to be negatively coreelated with the percentage of being unemployed, at -.58.
...
ANSWER HERE
I think the model does an okay job of predicting turnout, as it highlights two variables which I believe to be significant: income and the percentage of having a bachelors or higher. Both of these show a correlation, with the percentage of having a bachelors or higher being the strongest correlation at .83. I feel like the populaiton variable might be a little insignificant, as its hard for me to draw any noteworthy conslusions based on the relatively strong correlation it shows. Overall, i think the model can be helpful for understanding voter turnout, but can be made more clear by better choice in variables.
...
ANSWER HERE
My model two, which was found by just removing the population, is slightly better than the full model. My model 4, which only looks at the percentage of white voters and their age, is significantly worse than any model based on adjusted R-squared values.
...
# Model 3
model3 <- lm(turnout ~ pct_bachelors_higher + 
                   pct_unemployed + median_age + pct_white, data = md_turnout)
summary(model3)
...
ANSWER HERE
Recent analysis of multiple factors found that higher income residents with bachelor's degrees or higher were ht emost likely to come out to vote this election.